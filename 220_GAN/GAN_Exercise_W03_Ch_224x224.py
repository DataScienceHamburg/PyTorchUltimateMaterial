...
# In[1]:
## GAN Exercise With 03 Color Channels (3, 224, 224)
import numpy as np
import pandas as pd
...
##################### Training  #####################
## Training the discriminator and Generator in alternance
    ...
    for batch_idx, (real_samples, _) in enumerate(train_loader):
        ...
        if epoch % 1000 == 0:
            ...
            with torch.no_grad():
                latent_space_samples = torch.randn((BATCH_SIZE, z_dim))
                generated_samples = gen(latent_space_samples)
                generated_samples_processed = generated_samples.view(3, 224, 224).detach()
                generated_samples_processed = 0.5 * (generated_samples_processed +1)## Denormalize
                generated_samples_processed = generated_samples_processed.view(224, 224, 3)
                #plt.imshow(generated_samples_processed, cmap='viridis')
                plt.imshow(generated_samples_processed)##
                plt.grid(False)
                plt.axis('off')
                plt.savefig(f"./saved_images/saved_fig_GAN_03Ch_{epoch}_{NUM_EPOCHS}_224_224.png")